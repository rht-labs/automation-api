...
public interface UsersApi {
    ...
    @ApiResponses(value = { @ApiResponse(code = 400, message = "Invalid User value", response = Void.class) })
    ...
    ResponseEntity<Void> updateUser(@ApiParam(value = "User ID", required = true) @PathVariable("id") Long id
    ...
    @ApiOperation(value = "", notes = "Gets `User` objects. Optional query param of **size** determines size of returned array ", 
            response = User.class, responseContainer = "List", tags = {"user", })
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Successful response", response = User.class),
    ...
    ResponseEntity<User> usersIdGet(@ApiParam(value = "User ID", required = true) @PathVariable("id") Long id
