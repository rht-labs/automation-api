...
public interface EngagementSummariesApi {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "collection of `Engagement` objects", response = EngagementSummary.class),
            @ApiResponse(code = 400, message = "bad request: invalid syntax", response = EngagementSummary.class),
            @ApiResponse(code = 200, message = "unexpected error", response = EngagementSummary.class)})
